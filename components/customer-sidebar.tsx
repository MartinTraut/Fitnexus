...
import {
  LayoutDashboard, Search, Calendar, MessageCircle,
  TrendingUp, User, Settings, LogOut,
} from 'lucide-react'

const navItems = [
  { href: '/dashboard/customer', icon: LayoutDashboard, label: 'Dashboard' },
  { href: '/dashboard/customer/search', icon: Search, label: 'Suche' },
  { href: '/dashboard/customer/bookings', icon: Calendar, label: 'Buchungen' },
  { href: '/dashboard/customer/messages', icon: MessageCircle, label: 'Nachrichten' },
...
export function CustomerSidebar() {
  ...
  const handleLogout = () => {
    if (typeof window !== 'undefined') {
      localStorage.removeItem('fitnexus_user')
    }
    router.push('/')
  }
  ...
  return (
    <aside className="hidden md:flex md:w-64 md:flex-col md:fixed md:inset-y-0 z-40">
      <div className="flex flex-col flex-1 bg-[#0D1320] border-r border-[rgba(0,168,255,0.1)]">
        {/* Logo */}
        <div className="flex items-center justify-center h-20 px-6 border-b border-[rgba(0,168,255,0.08)]">
        ...
        <div className="px-3 py-4 border-t border-[rgba(0,168,255,0.08)] space-y-1">
          <Link
            ...
            className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-muted-foreground hover:text-foreground hover:bg-[#1A2332] transition-all duration-200"
          >
